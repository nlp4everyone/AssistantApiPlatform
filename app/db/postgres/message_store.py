...
class PostgresMessageStore:
    ...
    async def insert_messages(pool: asyncpg.Pool,
        ...
        if len(messages_data) == 1:
            ...
            async with pool.acquire() as conn:
                ...
                thread_exists = await conn.fetchval("SELECT 1 FROM threads WHERE id = $1", thread_id)
                # Raise error if thread not found
                if thread_exists is None: raise ThreadNotFoundException(thread_id = thread_id)
    ...
    async def get_thread_messages(pool: asyncpg.Pool,
        ...
        async with pool.acquire() as conn:
            ...
            # Raise error if thread not found
            if not exists: raise ThreadNotFoundException(thread_id = thread_id)
    ...
    async def get_message_by_id(pool: asyncpg.Pool,
        ...
        async with pool.acquire() as conn:
            ...
            exists = await conn.fetchval(
                "SELECT 1 FROM threads WHERE id = $1", thread_id
            )
            # Raise error if thread not found
            if not exists: raise ThreadNotFoundException(thread_id = thread_id)
            ...
            if row is None: raise MessageNotFoundException(message_id = message_id)
    ...
    async def delete_message(pool: asyncpg.Pool,
        ...
        async with pool.acquire() as conn:
            ...
            if result.endswith("0"): raise MessageNotFoundException(message_id = message_id)
